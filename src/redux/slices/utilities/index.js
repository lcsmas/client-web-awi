...
export const createFetchableSlice = ({ name, initialState, reducers }) => {
    ...
    let slice = createSlice({
        ...
        reducers: {
                    ...
                    for (const id in sliceDatas) {
                        ...
                        sliceMapping.forEach(fieldMapping => {
                            ...
                            state.byIds = {
                                ...state.byIds,
                                [id]:
                                    { ...state.byIds[id], [clientFieldName]: sliceData[serverFieldName] }
                            }
                        })
                    }
            ...
            postBegin: state => { state.fetchState.isPosting = true },
            postFailure: {
                reducer: (state, action) => {
                    state.fetchState.isPosting = false;
                    state.fetchState.error = action.payload.error;
            ...
            updateBegin: state => { state.fetchState.isUpdating = true },
            updateFailure: {
                reducer: (state, action) => {
                    state.fetchState.isUpdating = false;
                    state.fetchState.error = action.payload.error;
    ...
    slice.thunks = {
        fetch: () => dispatch => {
            ...
            return API.fetchSlice(name)
                .catch(err => dispatch(slice.actions.fetchFailure(err.message)))
                .then(res => dispatch(slice.actions.fetchSuccess(res)))
        },
        post: data => dispatch => {
            dispatch(slice.actions.postBegin())
            return API.postSlice(name, data)
                .catch(err => dispatch(slice.actions.postFailure(err.message)))
                .then(res => dispatch(slice.actions.postSuccess(res)))
        },
